...
def _iou_per_anchor(pred_boxes: FloatType,
    ...
    t_xmin, t_ymin, t_xmax, t_ymax = target_boxes
    p_xmin, p_ymin, p_xmax, p_ymax = pred_boxes
    ...
    p_center = tf.stack(
        [(p_ymin + p_ymax) / 2, (p_xmin + p_xmax) / 2], axis=-1)
    t_center = tf.stack(
        [(t_ymin + t_ymax) / 2, (t_xmin + t_xmax) / 2], axis=-1)
